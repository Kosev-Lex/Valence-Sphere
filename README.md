...
\`OPENAI\_API\_KEY\` can be used instead of \`VS\_API\`.

Only the API key is required when using the default OpenAI endpoint. \`VS\_MODEL\` and \`VS\_BASE\_URL\` are optional.
...
5\. Records the result in \`socrates\_lemon.json\`.
...
4\. Records the result in \`answerer\_lemon.json\`.
...
7\. Records the result in \`adjudication\_lemon.json\`.
...
└── lemon/ 
 ├── lemon.json 
 ├── socrates\_lemon.json 
 ├── answerer\_lemon.json 
 └── adjudication\_lemon.json 
\`\`\`
